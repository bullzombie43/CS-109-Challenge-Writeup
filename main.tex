...
\subsubsection*{f5 only:}
  ...
  \hline
  \end{tabular}
  \caption{Results comparison — f5 only (n=80 test videos). \textit{Accuracy}: fraction correctly classified. \textit{AUC}: area under the ROC curve, measures ranking ability independent of threshold. \textit{Precision}: of predicted fakes, fraction truly fake. \textit{Recall}: of all true fakes, fraction caught. \textit{F1}: harmonic mean of precision and recall. Baselines: \textit{Majority class} always predicts ``fake''; \textit{Random} flips a fair coin per video. See Appendix~\ref{app:figures} for confusion matrices and ROC curves.}
